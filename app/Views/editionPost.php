<main class="blog-content row d-flex flex-column justify-content-between my-5">
  <section class="text-editor-container container d-flex flex-column">
    ...
    <div class="buttons-row row d-flex justify-content-center mb-4">
      ...
      <form action="" method="post" class="container d-flex justify-content-start col-6 ml-0">
        <button type="submit" class="update-post btn btn-success btn-lg" name="update-post">Mettre à jour</button>
        <?php if ($tpl->viewVars['post']->getStatus() != 1) : ?>
        <button type="submit" class="publish-post btn btn-primary btn-lg " name="publish-post">Publier</button>
        <?php endif; ?>      
      </form>
    </div>
  </section>
