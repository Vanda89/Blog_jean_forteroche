...
var textpattern = (function () {
  ...
  var $_hbubwr3jikkeb6z = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_9npek4rbjikkeb77.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_9npek4rbjikkeb77.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_9npek4rbjikkeb77.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_5wxgsvr8jikkeb74.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_5wxgsvr8jikkeb74.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_5wxgsvr8jikkeb74.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_folsslr7jikkeb72.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_folsslr7jikkeb72.checkKeyCode(keyCodes, e)) {
        $_folsslr7jikkeb72.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_folsslr7jikkeb72.checkCharCode(charCodes, e)) {
  ...
  var $_7wplzor4jikkeb70 = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_hbubwr3jikkeb6z.getPatterns(editor.settings));
    $_7wplzor4jikkeb70.setup(editor, patternsState);
    return $_alo2j9r2jikkeb6y.get(patternsState);
  });
  function Plugin () {
  }
