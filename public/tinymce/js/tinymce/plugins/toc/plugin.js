...
var toc = (function () {
  ...
  var $_81z8vtrkjikkeb7v = { create: create };

  var tocId = $_81z8vtrkjikkeb7v.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_etmiqkrjjikkeb7u.getTocClass(editor);
    var headerTag = $_etmiqkrjjikkeb7u.getTocHeader(editor);
    var selector = generateSelector($_etmiqkrjjikkeb7u.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_etmiqkrjjikkeb7u.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_etmiqkrjjikkeb7u.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_fwyhnqrfjikkeb7r.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_fwyhnqrfjikkeb7r.updateToc(editor);
    });
  };
  var $_axujswrejikkeb7p = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_etmiqkrjjikkeb7u.getTocClass(editor);
  ...
  var $_d1h1hdrljikkeb7w = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_fwyhnqrfjikkeb7r.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_etmiqkrjjikkeb7u.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_335arfrmjikkeb7x = { register: register$1 };

  global.add('toc', function (editor) {
    $_axujswrejikkeb7p.register(editor);
    $_335arfrmjikkeb7x.register(editor);
    $_d1h1hdrljikkeb7w.setup(editor);
  });
  function Plugin () {
  }
